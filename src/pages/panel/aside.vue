<template>
  ...
  >
    ...
    <!-- 无人机页面-菜单 -->
    <el-submenu index="flying">
       <template #title>
        <sd-icon value="drone-blue"></sd-icon>
        <span>无人机页面</span>
      </template>
      <el-menu-item
        v-for="fly in flyList"
        :key="fly.info.id"
        :index="`node-${fly.info.id}`"
        v-if="fly.info.id === 1"
        :route="{ name: 'flying', params: { id: fly.info.id } }"
      >{{ fly.info.name }}</el-menu-item>
      <el-menu-item
        v-for="fly in flyList"
        :key="fly.info.id"
        :index="`node-${fly.info.id}`"
        v-if="fly.info.id === 2"
        :route="{ name: 'suspend', params: { id: fly.info.id } }"
      >{{ fly.info.name }}</el-menu-item>
    </el-submenu>
